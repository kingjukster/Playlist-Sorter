...
| r4 | Descriptors, lyrics handling, pinned lazy model adapters, provenance cache and no-inference reruns | `features/`, `embeddings/registry.py`, `configs/license_registry.toml` | feature and injected-adapter tests | Deterministic adapters/cache are implemented and synthetic-validated. Injected-adapter warm reruns skip inference; actual MuQ/MuLan/Qwen inference, numerical equivalence, and weight licenses in use remain unqualified. |
...
| r8 | Local-first privacy, ignore rules, pinned model/license boundaries, no source mutation or external writes | `.gitignore`, `LICENSE`, `configs/license_registry.toml`, `embeddings/registry.py` | source-byte invariance and repository diff inspection | Implemented policy and synthetic source-mutation checks. Commercial model profile and streaming writes are out of scope/unqualified. |
