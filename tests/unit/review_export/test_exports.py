...
def _run(tmp_path):
    ...
    (run / "songs.json").write_text(
        json.dumps({"schema_version": "1.0", "items": [_song("b", r"C:\Music\B.mp3"), _song("a", r"D:\Music\A.mp3")]}),
        encoding="utf-8",
    )
    (run / "playlists.json").write_text(
        json.dumps({"schema_version": "1.0", "items": [_playlist("z", ["b", "a"])]}), encoding="utf-8"
    )
    (run / "memberships.json").write_text(json.dumps({"schema_version": "1.0", "items": []}), encoding="utf-8")
    (run / "manifest.json").write_text(json.dumps({"schema_version": "1.0", "run_id": "r", "created_at": "2026-01-01T00:00:00Z", "discovery_run": "d", "artifacts": [], "candidates": ["z"], "generator_version": "1.0"}), encoding="utf-8")
    return run
...
def test_malformed_artifacts_and_unsafe_paths_fail_before_export(tmp_path):
    ...
    bad = tmp_path / "bad"
    bad.mkdir()
    (bad / "songs.json").write_text(json.dumps({"schema_version": "1.0", "items": [_song("a", r"C:\Music\..\secret.mp3")]}), encoding="utf-8")
    (bad / "playlists.json").write_text(json.dumps({"schema_version": "1.0", "items": [_playlist("p", ["a"])]}), encoding="utf-8")
    (bad / "memberships.json").write_text(json.dumps({"schema_version": "1.0", "items": []}), encoding="utf-8")
    (bad / "manifest.json").write_text(json.dumps({"schema_version": "1.0", "run_id": "r", "created_at": "2026-01-01T00:00:00Z", "discovery_run": "d", "artifacts": [], "candidates": ["p"], "generator_version": "1.0"}), encoding="utf-8")
...
def test_manifest_and_unknown_playlist_members_fail_closed(tmp_path):
    ...
    with pytest.raises(ExportError, match="canonical"):
        load_export_data(run)
    (run / "manifest.json").write_text(json.dumps({"schema_version": "1.0", "run_id": "r", "created_at": "2026-01-01T00:00:00Z", "discovery_run": "d", "artifacts": [], "candidates": ["p"], "generator_version": "1.0"}), encoding="utf-8")
    (run / "playlists.json").write_text(json.dumps({"schema_version": "1.0", "items": [_playlist("p", ["missing"])]}), encoding="utf-8")
    with pytest.raises(ExportError, match="canonical"):
        load_export_data(run)
