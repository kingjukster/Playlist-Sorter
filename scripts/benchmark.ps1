...
param(
    ...
    [string]$Output = "./runs/benchmark-200",
...
$SmokeReceiptPath = [System.IO.Path]::GetFullPath($SmokeReceipt)
if (-not (Test-Path -LiteralPath $SmokeReceiptPath -PathType Leaf)) {
    throw "A completed smoke receipt is required: $SmokeReceiptPath"
}
$Smoke = Get-Content -LiteralPath $SmokeReceiptPath -Raw | ConvertFrom-Json
if ($Smoke.schema_version -ne "1.0" -or $Smoke.gate -ne "20-item-smoke" -or
    $Smoke.passed -ne $true -or
    $Smoke.result.expected_items -ne 20 -or $Smoke.result.available_items -ne 20) {
    throw "Smoke receipt does not prove a passed exact 20-item gate."
}
$SmokeReceiptSha256 = (Get-FileHash -LiteralPath $SmokeReceiptPath -Algorithm SHA256).Hash.ToLowerInvariant()
...
if ($AudioFiles.Count -ne 200) {
    throw "The scale gate requires exactly 200 supported audio files; found $($AudioFiles.Count)."
}
...
if ($Smoke.git_commit -ne $GitCommit -or
    $Smoke.repository_aggregate_sha256 -ne $RepositoryAggregateSha256 -or
    $Smoke.distribution -ne $Distribution) {
    throw "Smoke receipt is not bound to this exact commit, repository bytes, and WSL distribution."
}
...
$WslArguments = @(
    ...
    "python3.12", "-c", $PythonCode, $WslLibrary, "200"
)
...
$Receipt = [ordered]@{
    ...
    library = $LibraryPath
    output = $OutputPath
    prerequisite = [ordered]@{ path = $SmokeReceiptPath; sha256 = $SmokeReceiptSha256 }
    result = $Result
}
$ReceiptPath = Join-Path $OutputPath "benchmark-receipt.json"
...
@(
    ...
    "- Smoke receipt SHA-256: $SmokeReceiptSha256"
    "- Expected/scanned: 200/$($Result.available_items)"
...
if (-not $Passed) {
    throw "200-item scale gate failed; see $ReceiptPath"
}
